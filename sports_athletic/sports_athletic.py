...
class AttachmentHistoryAthlete(models.Model):

    _name = "attachment.history.athlete"
    _description = "Historique des PJ"

    date = fields.Date(string='Date', default=lambda *a: time.strftime("%Y-%m-%d"))
    attachment = fields.Binary("Document", required=True)
    attachment_filename = fields.Char("Nom document")
    type = fields.Selection([('birth_certificate', 'Extrait acte de naissance'),
                             ('medical_certificate', 'Cértificat médical'),
                             ('cin', 'CIN pour les séniors et Juniors'),
                             ('parental_authorization', 'Autorisation des parents'),
                             ('residence_permit', 'Carte de séjour'),
                             ('foreign_authorization', 'Autorisation étrangère'),
                             ('school_certificate', 'Cértificat de scolarité'),
                             ('school_att', 'Attestation de scolarité')
                             ],
                                     'Type document', required=True)
    club = fields.Many2one('sports.club', string='Club')
    athlete = fields.Many2one('sports.athletes', string='Athlète')

AttachmentHistoryAthlete()


class ClubGeneralAssembly(models.Model):

    _name = "club.general.assembly"
    _description = u"Assemblée Générale"

    @api.model
    def _get_default_saison(self):
        return self.env['sports.saison'].search([('state', '=', 'active')])

    name = fields.Char(string="Code de l'Assemblée", readonly=True)
    saison_id = fields.Many2one('sports.saison', string='Saison sportive', default=_get_default_saison, required=True)
    ligue_id = fields.Many2one('sports.ligue', string='Ligue')
    club_id = fields.Many2one('sports.club', string='Club', required=True)
    start_date = fields.Date(string='Date début')
    end_date = fields.Date(string='Date fin')
    deadline = fields.Date(string='Date Butoir pour Réception des Dossiers Administratifs')
    assembly_nature = fields.Selection([('Ordinaire', 'Ordinaire'), ('Extra Ordinaire', 'Extra Ordinaire')],
                                       string="Nature de l'assemblée", default="")
    assembly_place = fields.Char(string="Lieu de l'Assemblée")
    assembly_date = fields.Date(string="Date de l'Assemblée")
    agenda = fields.Text(string="Ordre du jour")
    league_representative = fields.Char(string="Représentant Ligue")
    note = fields.Text(string="Note")
    league_representative_report = fields.Binary("Rapport Représentant Ligue")
    ...
    financial_report_view_comment = fields.Text(string='Commentaire')
    commission_decision = fields.Selection([('Non Conforme', 'Non Conforme'), ('Conforme', 'Conforme')],
                                           string="Décision de la Commission", default="")
    commission_decision_comment = fields.Text(string='Motif')

    state = fields.Selection([
        ("rejected", "Rejeté"),
        ("waiting_planning", "En Attente de Planification de l'AG"),
        ("waiting_approval", "En Attente de l'Accord de Principe"),
        ("waiting_report_frma", "En Attente du Compte Rendu du Représentant de la Ligue"),
        ("waiting_admin_record", "En Attente du Dossier Administratif"),
        ("waiting_admin_record_paper", "En Attente du Dossier Administratif Papier"),
        ("waiting_fin_report", "En Attente de Validation du Rapport Financier"),
        ("waiting_admin_record_study", "En attente de décision de la commission"),
        ("admin_status_updated", "Situation Administrative à Jour")],
        string="Statut", default="waiting_planning")

    planned_time = fields.Datetime("Planification AG le", readonly=True)
    ...
    def action_waiting_approval(self):
        self.write({'state': 'waiting_approval',
                    'planned_time': time.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                    'planned_user_id': self.env.uid})

    @api.one
    def action_waiting_report_frma(self):
        self.write({'state': 'waiting_report_frma',
                    'approved_time': time.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                    'approved_user_id': self.env.uid})

    @api.one
    def action_waiting_admin_record(self):
        self.write({'state': 'waiting_admin_record',
                    'reported_time': time.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                    'reported_user_id': self.env.uid})

    @api.one
    def action_waiting_admin_record_paper(self):
        self.write({'state': 'waiting_admin_record_paper',
                    'loaded_num_time': time.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                    'loaded_num_user_id': self.env.uid})

    @api.one
    def action_waiting_fin_report(self):
        self.write({'state': 'waiting_fin_report',
                    'loaded_paper_time': time.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                    'loaded_paper_user_id': self.env.uid})

    @api.one
    def action_waiting_admin_record_study(self):
        self.write({'state': 'waiting_admin_record_study',
                    'validated_time': time.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                    'validated_user_id': self.env.uid})

    @api.one
    def action_admin_status_updated(self):
        self.write({'state': 'admin_status_updated',
                    'decided_time': time.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                    'decided_user_id': self.env.uid})

    @api.onchange('league_representative_report_reception')
    def onchange_league_representative_report_reception(self):
        self.league_representative_report_reception_date = False
        if self.league_representative_report_reception:
            self.league_representative_report_reception_date = time.strftime('%Y-%m-%d')

    @api.onchange('minutes_reception')
    def onchange_minutes_reception(self):
        self.minutes_reception_date = False
        if self.minutes_reception:
            self.minutes_reception_date = time.strftime('%Y-%m-%d')

    @api.onchange('attendance_list_reception')
    def onchange_attendance_list_reception(self):
        self.attendance_list_reception_date = False
        if self.attendance_list_reception:
            self.attendance_list_reception_date = time.strftime('%Y-%m-%d')

    @api.onchange('financial_report_reception')
    def onchange_financial_report_reception(self):
        self.financial_report_reception_date = False
        if self.financial_report_reception:
            self.financial_report_reception_date = time.strftime('%Y-%m-%d')

    @api.onchange('technical_report_reception')
    def onchange_technical_report_reception(self):
        self.technical_report_reception_date = False
        if self.technical_report_reception:
            self.technical_report_reception_date = time.strftime('%Y-%m-%d')

    @api.onchange('signature_specimen_reception')
    def onchange_signature_specimen_reception(self):
        self.signature_specimen_reception_date = False
        if self.signature_specimen_reception:
            self.signature_specimen_reception_date = time.strftime('%Y-%m-%d')

    @api.onchange('recipe_deposit_reception')
    def onchange_recipe_deposit_reception(self):
        self.recipe_deposit_reception_date = False
        if self.recipe_deposit_reception:
            self.recipe_deposit_reception_date = time.strftime('%Y-%m-%d')

    @api.model
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('general.assembly')
        return super(ClubGeneralAssembly, self).create(vals)
...
class LigueGeneralAssembly(models.Model):

    _name = "ligue.general.assembly"
    _description = u"Assemblée Générale"
    ...
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('general.assembly')
        return super(LigueGeneralAssembly, self).create(vals)



LigueGeneralAssembly()
...
class GeneralAssemblyMember(models.Model):
    ...
    _name = "general.assembly.member"
    _description = "Membre"
    _rec_name = "reference"


    assembly_id = fields.Many2one('club.general.assembly', string='Assemblé générale club')
    reference = fields.Char(string="Séquence", readonly=True)
    fname = fields.Char(string="Nom", required=True)
    lname = fields.Char(string="Prénom", required=True)
    cin = fields.Char(string="CIN")
    sex = fields.Selection([('Masculin', 'Masculin'), (u'Féminin', u'Féminin')],
                           string="Sexe", default="")
    job = fields.Char(string="Profession")
    dob = fields.Char(string="Date de naissance")
    city = fields.Char(string="Lieu de naissance")
    mobile = fields.Char(string="Mobile")
    mail = fields.Char(string="Courriel")
    marital_status = fields.Selection([(u'Célibataire', u'Célibataire'), (u'Marié', u'Marié'),
                                       (u'Divorcé', u'Divorcé')],
                                      string="Statut Marital", default="")
    children_nbr = fields.Integer(string="Nombre d'enfants")
    ...
    @api.model
    def create(self, vals):
        vals['reference'] = self.env['ir.sequence'].next_by_code('general.assembly.member')
        return super(GeneralAssemblyMember, self).create(vals)
...
class SportsSaison(models.Model):
    ...
    @api.one
    def action_make_club_compliant(self):
        clubs = self.env['sports.club'].search([('id', '>=', 0)])
        for club in clubs:
            club.situation_admin = "Conforme"

    @api.multi
    def action_cloturee(self):
        if self.make_club_non_compliant:
            clubs = self.env['sports.club'].search([('id', '>=', 0)])
            for club in clubs:
...
class SportsLigue(models.Model):
    ...
    situation_admin = fields.Selection([
        ...
        ('Conforme', 'Conforme'),
    ], string='Situation Administrative', default="Non Conforme")
...
class SportsCategory_age(models.Model):
    ...
    parental_authorization = fields.Boolean(string="Autorisation des parents")
    residence_permit = fields.Boolean(string="Carte de séjour")
    foreign_authorization = fields.Boolean(string="Autorisation étrangère")
    school_certificate = fields.Boolean(string="Cértificat de scolarité")
    school_att = fields.Boolean(string="Attestation de scolarité")
...
class SportsAffiliation(models.Model):
    ...
    @api.one
    @api.constrains('athletes_ids')
    def _check_athletes_ids_name(self):
        for athlete1 in self.athletes_ids:
            i = 0
            for athlete2 in self.athletes_ids:
                if athlete1.name_at == athlete2.name_at and athlete1.prenom == athlete2.prenom:
                    i += 1
            if i > 1:
                raise ValidationError("Athlète Doubon détécté dans la liste des nouveaus attribution, merci de vérifier.\n"
                                      "Nom : " + athlete1.name_at.encode('utf_8') + " " + athlete1.prenom.encode('utf_8'))
    ...
    def action_verification(self):
        ...
        # for a in athletes:
        #     a.double = False
        # ad = self.env['sports.doublon'].search([])
        # if ad:
        #     ad.unlink()
        ...
        if self.athletes_ids:
            ...
            for athletes in self.athletes_ids:
                if athletes.name == 'Nouveau':
                    ...
                    if athletes.licence == "Officiel":
                        name = self.env['ir.sequence'].next_by_code('sports.official')
                    athletes.write({'state': 'actif', 'name': name})
    ...
    def action_creation(self):
        if self.athletes3_ids:
            ...
            for athletes in self.athletes3_ids:
                ...
                if athletes.state != "nonConforme":
                    if self.affilia_athlete:
                        if athletes.cinjuniorsseniors:
                            if athletes.cinjuniorsseniors:
                                self.env['attachment.history.athlete'].create({
                                    'athlete': athletes.id,
                                    'type': 'cin',
                                    'attachment_filename': "CIN pour les séniors et Juniors" + ".jpg",
                                    'attachment': athletes.cinjuniorsseniors,
                                    'club': athletes.club_id.id,
                                })
                                athletes.cinjuniorsseniors = self.affilia_athlete.cinjuniorsseniors

                        if athletes.certificatmedical:
                            if athletes.certificatmedical:
                                self.env['attachment.history.athlete'].create({
                                    'athlete': athletes.id,
                                    'type': 'cin',
                                    'attachment_filename': "Cértificat Médical" + ".jpg",
                                    'attachment': athletes.certificatmedical,
                                    'club': athletes.club_id.id,
                                })
                                athletes.certificatmedical = self.affilia_athlete.certificatmedical

                        if athletes.cartesejour:
                            if athletes.cartesejour:
                                self.env['attachment.history.athlete'].create({
                                    'athlete': athletes.id,
                                    'type': 'cin',
                                    'attachment_filename': "CIN pour les séniors et Juniors" + ".jpg",
                                    'attachment': athletes.cartesejour,
                                    'club': athletes.club_id.id,
                                })
                                athletes.cartesejour = self.affilia_athlete.cartesejour

                        if athletes.autorisationparents:
                            if athletes.autorisationparents:
                                self.env['attachment.history.athlete'].create({
                                    'athlete': athletes.id,
                                    'type': 'cin',
                                    'attachment_filename': "Autorisation des Parents" + ".jpg",
                                    'attachment': athletes.autorisationparents,
                                    'club': athletes.club_id.id,
                                })
                                athletes.autorisationparents = self.affilia_athlete.autorisationparents

                        if athletes.autorisationetrangere:
                            if athletes.autorisationetrangere:
                                self.env['attachment.history.athlete'].create({
                                    'athlete': athletes.id,
                                    'type': 'cin',
                                    'attachment_filename': "Autorisation étrangère" + ".jpg",
                                    'attachment': athletes.autorisationetrangere,
                                    'club': athletes.club_id.id,
                                })
                                athletes.autorisationetrangere = self.affilia_athlete.autorisationetrangere

                        if athletes.certificatscolarite:
                            if athletes.certificatscolarite:
                                self.env['attachment.history.athlete'].create({
                                    'athlete': athletes.id,
                                    'type': 'cin',
                                    'attachment_filename': "Cértificat de Scolarité" + ".jpg",
                                    'attachment': athletes.certificatscolarite,
                                    'club': athletes.club_id.id,
                                })
                                athletes.certificatscolarite = self.affilia_athlete.certificatscolarite

                        if athletes.attestationscolarite:
                            if athletes.attestationscolarite:
                                self.env['attachment.history.athlete'].create({
                                    'athlete': athletes.id,
                                    'type': 'cin',
                                    'attachment_filename': "Attestation de Scolarité" + ".jpg",
                                    'attachment': athletes.attestationscolarite,
                                    'club': athletes.club_id.id,
                                })
                                athletes.attestationscolarite = self.affilia_athlete.attestationscolarite

                        if athletes.actenaissance:
                            if athletes.actenaissance:
                                self.env['attachment.history.athlete'].create({
                                    'athlete': athletes.id,
                                    'type': 'cin',
                                    'attachment_filename': "Extrait d'acte de Naissance" + ".jpg",
                                    'attachment': athletes.actenaissance,
                                    'club': athletes.club_id.id,
                                })
                                athletes.actenaissance = self.affilia_athlete.actenaissance



                    athletes.write(
                        {'state': 'actif', 'club_id': self.club_id.id, 'ligue_id': self.ligue_id.id})
...
class SportsAthletes(models.Model):
    ...
    #     name = ""
    #     if self.name_at:
    #         name += (self.name_at).encode('utf_8')
    #     if self.prenom:
    #         name += (self.prenom).encode('utf_8')
    #     if self.licence == "Athlete":
    #         if self.category_age_id:
    #             if self.category_age_id.birth_certificate:
    #                 if not self.actenaissance:
    #                     msg += str(i) + ") " + name + " Veuillez remplir le champ obligatoire : Extrait acte de naissance.\n"
    #                     i += 1
    #             if self.category_age_id.cin:
    #                     msg += str(i) + ") " + name + " Veuillez remplir le champ obligatoire : CIN pour les Séniors et Juniors.\n"
    #
    #
    #         if msg:
    ...
    def view_athlete_doublon(self):
        ...
        view_ref = self.env['ir.model.data'].get_object_reference('sports_athletic', 'kanban_view_doublon')
        ...
        res = {
            ...
            'name': _('Liste des athlètes doublons'),
            'res_model': 'sports.doublon',
            'view_type': 'kanban',
            'view_mode': 'kanban',
            'view_id': view_id,
            'target': 'current',
        }
        if 'affiliation_id' in self._context:
            res['domain'] = str([('affiliation_id', '=', self._context['affiliation_id'])])
    ...
    def _name_prenom1(self):
        ...
        for order in self:
            if order.name_at:
                name_at = str(order.name_at).strip().upper()
                name_at =name_at.replace(',', '')
                name_at = name_at.replace('  ', ' ')
                order.name_at = name_at
            if order.prenom:
                prenom = str(order.prenom).strip().upper()
    ...
    category_age_parental_authorization = fields.Boolean(related="category_age_id.parental_authorization", string="Autorisation des parents")
    category_age_residence_permit = fields.Boolean(related="category_age_id.residence_permit", string="Carte de séjour")
...
class doublon(models.Model):
    ...
    affiliation_id = fields.Many2one('sports.affiliation', string='Affiliation')
...
class SportsSanctions(models.Model):
    ...
    def _ligue_id(self):
        if self.club_id:
            if self.club_id.ligue_id:
                ligue = self.env['sports.ligue'].search([('id', '=', self.club_id.ligue_id.id)])
                if ligue:
                    ligue = ligue[0]
                    self.ligue_id = ligue
...
class SportsMutations(models.Model):
    ...
    @api.one
    @api.constrains('athletes_id')
    def _check_athletes_id_raise(self):
        if self.athletes_id:
            ...
            if self.athletes_id.state == "Sanctionne":
                sanction = self.env['sports.sanctions'].search(
                    [('athletes_id', '=', self.athletes_id.id), ('saison_id', '=', saison.id),
                     ('state', '=', "athlete_sanctionne")])
                if sanction:
                    raise ValidationError(
                        "l'athlète est en état de sanction pour cette année sportive, vous ne pouvez pas effectuer cette action.")
            if self.athletes_id.state == "AnneeBlanche":
                ab = self.env['sports.anneeblanche'].search(
                    [('athletes_id', '=', self.athletes_id.id), ('saison_id', '=', saison.id),
                     ('state', '=', "athlete_sanctionne")])
                if ab:
                    raise ValidationError(
                        "l'athlète possède une année blanche pour cette année sportive, vous ne pouvez pas effectuer cette action.")
    ...
    def _change_nom_prenom(self):
        self.name_at = self.athletes_id.name_at
        self.prenom = self.athletes_id.prenom
...
class SportsAnneeblanche(models.Model):
    ...
    @api.one
    @api.constrains('athletes_id')
    def _check_athletes_id_raise_2(self):
        if self.athletes_id:
            saison = self.env['sports.saison'].search([('state', '=', 'active')])
            if not saison:
                raise ValidationError("Auncun saison active, merci de contacter l'administrateur.")
    ...
    def create(self, vals):
        ...
        result = super(SportsAnneeblanche, self).create(vals)
        return result

    @api.one
    def action_anneeblanche_validate(self):
        self.write({'state': 'attente_verif_csrl',
                    'user_attente_envoi_demande': self.env.uid,
                    'date_attente_envoi_demande': time.strftime(DEFAULT_SERVER_DATETIME_FORMAT),})

    @api.one
    def action_anneeblanche_validate_csrl(self):
        self.write({'state': 'attente_decision_comm',
                    'user_attente_verif_csrl': self.env.uid,
                    'date_attente_verif_csrl': time.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                    })
    ...
    @api.one
    def action_anneeblanche_validate_commission(self):
        self.write({'state': 'athlete_sanctionne',
                    'user_attente_decision_comm': self.env.uid,
                    'date_attente_decision_comm': time.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                    })
        self.athletes_id.write({'state': 'AnneeBlanche'})
...
class Editionpassagecategorie(models.Model):
    ...
    type_ = fields.Selection([('Tout', 'Tout'), ('avec_changement_categorie', 'Avec changement de catégorie'),
    ...
    def action_verification(self):
        ...
        if not self.passage_ids:
            ...
            for club in self.club_ids:
                ...
                if self.type_ == 'sans_changement_categorie':
                ...
                if self.type_ == 'Tout':
                    ...
                    athletes_ids = self.env['sports.athletes'].search([('club_id', '=', club.id)])
    ...
    @api.multi
    def _get_club_ml(self):
        res = []
        club = []
        for c in self.club_ids:
            club.append(c.id)
        for passage in self.passage_ids:
            if passage.athletes_id.club_id.id in club:
                if passage.athletes_id.club_id not in res:
                    res.append(passage.athletes_id.club_id)
        return res

    @api.multi
    def _get_age_category(self):
        res = []
        return self.env['sports.category_age'].search([], order="order asc")

    @api.multi
    def _get_athletes_list(self, club, category):
        res = []
        passage_ids = self.passage_ids.search([('club_id', '=', club.id), ('category_age_id', '=', category.id)])
        for passage in passage_ids:
            if passage.athletes_id:
                res.append(passage.athletes_id)
        return res
